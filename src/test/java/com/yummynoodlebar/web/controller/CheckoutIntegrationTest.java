...
public class CheckoutIntegrationTest {
	...
	private static final String CHECKOUT_VIEW = "/WEB-INF/views/checkout.jade";
	...
	public void thatSendsCorrectOrderEventOnSuccess() throws Exception {
		...
		verify(orderService).createOrder(Matchers.<CreateOrderEvent>argThat(
				allOf(
						org.hamcrest.Matchers.<CreateOrderEvent>hasProperty("details", hasProperty("dateTimeOfSubmission", notNullValue())),
						org.hamcrest.Matchers.<CreateOrderEvent>hasProperty("details", hasProperty("name", equalTo(CUSTOMER_NAME))),
						org.hamcrest.Matchers.<CreateOrderEvent>hasProperty("details", hasProperty("address1", equalTo(ADDRESS1))),
						org.hamcrest.Matchers.<CreateOrderEvent>hasProperty("details", hasProperty("postcode", equalTo(POST_CODE)))
				)
		));
		//@formatter:on
	}
